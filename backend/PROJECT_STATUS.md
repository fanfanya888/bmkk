...
- 前端结果比对图片请求已追加稳定版本参数，避免覆盖同名图片后继续命中旧缓存
- 后端已补充轻量结果概览聚合接口，按模型汇总首轮命中、二三轮累计命中、轮次覆盖和新增命中
- 前端首页已切换为结果概览页，原运行概览入口已移除；当前首页聚焦模型榜单与多轮累计命中
- 前端首页概览已支持导出当前榜单为独立 HTML 静态文件，且已移除低价值的 `api_style` / `新增命中` 展示
